...
- This rule also covers **reference data and UI strings that originate from the workbook/CSV sources** – progression tables, bonus tables, row/column headings, tab and field labels, and similar content. Seed them into the game database (defining a single structured source of truth in `src/heroforge/logic/` when no source file exists, mirroring `STANDARD_FAMILIAR_BONUSES` / `STANDARD_COMPANION_PROGRESSION`), expose them through `GameDataRepository`, and fetch them from the database at runtime. The in-code constant is only an offline fallback for an unseeded database, never the runtime source.
